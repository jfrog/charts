# Xray HA with Catalog and Valkey

This example shows how to deploy Xray in HA mode with Catalog, Valkey (with Sentinel), RabbitMQ quorum queues, and external PostgreSQL databases.
...
See the [values-xray-ha-with-catalog-and-valkey.yaml](values-xray-ha-with-catalog-and-valkey.yaml) for the configuration example.
...
```shell
helm upgrade --install xray jfrog/xray -f values-xray-ha-with-catalog-and-valkey.yaml -f xray-large.yaml
```

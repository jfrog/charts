...
* Aligned the default `replicaCount` of the `frontend`, `rtfs`, `jfbus`, `jfmelt`, `apptrust`, `unifiedpolicy`, `evaluation` and `platformfederation` Deployments with `artifactory.primary.replicaCount`, in `values.yaml` and in all sizing files.
...
* Added Platfrom Federation service

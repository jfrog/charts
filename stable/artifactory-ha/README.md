...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha center/jfrog/artifactory-ha --version 1.10.1
```
...
helm upgrade --install artifactory-ha --namespace artifactory-ha --set artifactory.replicator.enabled=true --set artifactory.replicator.publicUrl=<artifactory_url>:<replicator_port> center/jfrog/artifactory-ha
```
...
```bash
helm upgrade --install  <myrelease> --namespace artifactory-ha center/jfrog/artifactory-ha --set postgresql.postgresqlPassword=${POSTGRES_PASSWORD}
```
...
helm upgrade --install artifactory-ha --namespace artifactory-ha --set artifactory.masterKey=${MASTER_KEY} center/jfrog/artifactory-ha
```
...
helm upgrade --install artifactory-ha --namespace artifactory-ha --set artifactory.masterKeySecretName=my-secret center/jfrog/artifactory-ha
```
...
helm upgrade --install artifactory-ha --namespace artifactory-ha --set artifactory.license.secret=artifactory-cluster-license,artifactory.license.dataKey=art.lic center/jfrog/artifactory-ha
```
...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha -f values.yaml center/jfrog/artifactory-ha
```
**NOTE:** This method is relevant for initial deployment only! Once Artifactory is deployed, you should not keep passing these parameters as the license is already persisted into Artifactory's storage (they will be ignored).
Updating the license should be done via Artifactory UI or REST API.
...
```bash
helm upgrade --install artifactory-ha center/jfrog/artifactory-ha -f access-creds-values.yaml
```
...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha --set artifactory.license.secret=artifactory-cluster-license,artifactory.license.dataKey=art.lic,artifactory.configMapName=my-release-bootstrap-config center/jfrog/artifactory-ha
```
...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha --set nginx.customConfigMap=nginx-config center/jfrog/artifactory-ha
```
...
helm upgrade --install artifactory-ha --set artifactory.node.replicaCount=3 --set postgresql.postgresqlPassword=${DB_PASSWORD} center/jfrog/artifactory-ha
```
...
helm upgrade --install artifactory-ha --set artifactory.node.replicaCount=2 --set postgresql.postgresqlPassword=${DB_PASSWORD} center/jfrog/artifactory-ha
```
...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha --set imagePullSecrets=regsecret center/jfrog/artifactory-ha
```
...
```bash
helm upgrade --install artifactory-ha --namespace artifactory-ha -f plugins.yaml center/jfrog/artifactory-ha
```
...
and running:
```bash
helm upgrade --install artifactory-ha center/jfrog/artifactory-ha -f artifactory-ha-values.yaml
```

...
```bash
helm upgrade --install artifactory-cpp-ce jfrog/artifactory-cpp-ce \
    --namespace artifactory-cpp-ce --create-namespace \
...
```bash
kubectl create namespace artifactory-cpp-ce
kubectl create secret tls artifactory-nginx-tls \
    --cert=./tls.crt --key=./tls.key -n artifactory-cpp-ce
helm upgrade --install artifactory-cpp-ce jfrog/artifactory-cpp-ce \
    --namespace artifactory-cpp-ce --create-namespace \
    --set artifactory.postgresql.auth.password=<postgres_password> \
...
```bash
kubectl delete secret <release>-artifactory-nginx-certificate -n artifactory-cpp-ce
```

...
helm upgrade --set common.xrayVersion=3.6.2 jfrog/xray
```
...
If Xray was installed without providing a value to rabbitmq.auth.password (a password was autogenerated), follow these instructions:
1. Get the current password by running:
...
```bash
helm upgrade --install xray --namespace xray jfrog/xray --set rabbitmq.auth.password=<rabbit-password> --set postgresql.postgresqlPassword=<postgresql-password>
```
...
helm upgrade --install xray --set xray.masterKey=${MASTER_KEY} --namespace xray jfrog/xray
...
helm upgrade --install xray --namespace xray --set replicaCount=3  --set rabbitmq.replicaCount=3 jfrog/xray
```
...
Note: Support for custom certificates using secrets was added from 6.11.x chart versions

Tls certificates can be added by using kubernetes secret. The secret should be created outside of this chart and provided using the tag `.Values.xray.customCertificates.certificateSecretName`. Please refer the example below.

...
Install JFrog Xray
```bash
helm upgrade --install xray --namespace xray jfrog/xray --version 2.6.1
```
...
helm upgrade --install xray --namespace xray --set common.xrayVersion=2.16.14  jfrog/xray
```
...
```bash
helm upgrade --install <myrelease> --namespace xray  jfrog/xray --set postgresql.postgresqlPassword=${POSTGRES_PASSWORD}
```
...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade --install <myrelease> --namespace xray jfrog/xray --set rabbitmq.rabbitmqPassword=${RABBITMQ_PASSWORD}/rabbitmq-ha.rabbitmqPassword=${RABBITMQ_PASSWORD}
```
...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade --install <myrelease> --namespace xray jfrog/xray --set mongodb.mongodbRootPassword=${MONGODB_ROOT_PASSWORD} --set mongodb.mongodbPassword=${MONGODB_PASSWORD}
```
...
helm upgrade --install xray --set common.masterKey=${MASTER_KEY} -n xray jfrog/xray
```
...
helm upgrade --install xray -n xray --set analysis.replicaCount=3,server.replicaCount=3,indexer.replicaCount=3,persist.replicaCount=3,rabbitmq-ha.replicaCount=3 jfrog/xray
```
...
and running:
```bash
helm upgrade --install xray jfrog/xray --namespace xray -f xray-values.yaml
```

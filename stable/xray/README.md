...
Install JFrog Xray
```bash
helm upgrade --install xray --namespace xray center/jfrog/xray --version 2.5.3
```
...
```bash
# Update existing deployed version to 2.16.14
helm upgrade --install xray --namespace xray --set common.xrayVersion=2.16.14  center/jfrog/xray
```
...
```bash
helm upgrade --install <myrelease> --namespace xray  center/jfrog/xray --set postgresql.postgresqlPassword=${POSTGRES_PASSWORD}
```
...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade --install <myrelease> --namespace xray center/jfrog/xray --set rabbitmq.rabbitmqPassword=${RABBITMQ_PASSWORD}/rabbitmq-ha.rabbitmqPassword=${RABBITMQ_PASSWORD}
```
...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade --install <myrelease> --namespace xray center/jfrog/xray --set mongodb.mongodbRootPassword=${MONGODB_ROOT_PASSWORD} --set mongodb.mongodbPassword=${MONGODB_PASSWORD}
```
...
helm upgrade --install xray --set common.masterKey=${MASTER_KEY} -n xray center/jfrog/xray
```
...
helm upgrade --install xray -n xray --set analysis.replicaCount=3,server.replicaCount=3,indexer.replicaCount=3,persist.replicaCount=3,rabbitmq-ha.replicaCount=3 center/jfrog/xray
```
...
helm upgrade --install xray -n xray \
...
| `analysis.livenessProbe.path`                  | Xray Analysis liveness probe path                       | `/api/v1/system/ping`                  |
...
| `analysis.readinessProbe.path`                  | Xray Analysis readiness probe path                       | `/api/v1/system/ping`                  |
...
| `indexer.livenessProbe.path`                  | Xray Indexer liveness probe path                       | `/api/v1/system/ping`                  |
...
| `indexer.readinessProbe.path`                  | Xray Indexer readiness probe path                       | `/api/v1/system/ping`                  |
...
| `persist.livenessProbe.path`                  | Xray Persist liveness probe path                       | `/api/v1/system/ping`                  |
...
| `persist.readinessProbe.path`                  | Xray Persist readiness probe path                       | `/api/v1/system/ping`                  |
...
and running:
```bash
helm upgrade --install xray center/jfrog/xray --namespace xray -f xray-values.yaml
```

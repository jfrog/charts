...
helm upgrade --set common.xrayVersion=2.14.0 --namespace xray center/jfrog/xray
```
...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade <myrelease> center/jfrog/xray --set mongodb.mongodbRootPassword=${MONGODB_ROOT_PASSWORD} --set mongodb.mongodbPassword=${MONGODB_PASSWORD}
```
...
```bash
helm upgrade --name xray center/jfrog/xray \
...
helm install --set common.masterKey=${MASTER_KEY} -n xray center/jfrog/xray
```
...
helm install -n xray --set analysis.replicaCount=3,server.replicaCount=3,indexer.replicaCount=3,persist.replicaCount=3,rabbitmq-ha.replicaCount=3 center/jfrog/xray
```

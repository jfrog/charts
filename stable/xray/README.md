...
```bash
helm upgrade <myrelease> center/jfrog/xray --set rabbitmq.auth.password=${RABBITMQ_PASSWORD}
```
...
```bash
helm upgrade --install xray --namespace xray center/jfrog/xray --set rabbitmq.auth.password=<rabbit-password> --set postgresql.postgresqlPassword=<postgresql-password>
```
...
helm upgrade --install xray --namespace xray --set replicaCount=3  --set rabbitmq.replicaCount=3 center/jfrog/xray
```

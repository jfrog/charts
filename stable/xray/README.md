...
If Xray was installed without providing a value to rabbitmq.rabbitmqPasswoxray.rabbitmqPassword (a password was autogenerated), follow these instructions:
1. Get the current password by running:
...
```bash
helm upgrade <myrelease> center/jfrog/xray --set rabbitmq.rabbitmqPassword=${RABBITMQ_PASSWORxray.rabbitmqPassword=${RABBITMQ_PASSWORD}
```
...
```bash
helm upgrade --install xray --namespace xray center/jfrog/xray --sxray.rabbitmqPassword=<rabbit-password> --set postgresql.postgresqlPassword=<postgresql-password>
```
...
helm upgrade --install xray --namespace xray --set replicaCount=3  --sxray.replicaCount=3 center/jfrog/xray
```

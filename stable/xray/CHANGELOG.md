...
* **Breaking change: Valkey PVC size default increased from `1Gi` to `200Gi`**
  ...
  What changed: `valkey.primary.persistence.size` and `valkey.replica.persistence.size` both moved from `1Gi` to `200Gi`. With Sentinel enabled — the default since `103.150.37` — all Valkey nodes run in one StatefulSet whose PVC size comes from `valkey.replica.persistence.size`; `valkey.primary.persistence.size` has no effect in that mode.

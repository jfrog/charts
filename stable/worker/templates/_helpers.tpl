...
echo "Copy custom certificates to /var/opt/jfrog/worker/data/router/keys/trusted and /var/opt/jfrog/worker/etc/security/keys/trusted";
mkdir -p /var/opt/jfrog/worker/data/router/keys/trusted;
mkdir -p /var/opt/jfrog/worker/etc/security/keys/trusted;
for file in $(ls -1 /tmp/certs/* | grep -v .key | grep -v ":" | grep -v grep); do if [ -f "${file}" ]; then cp -v ${file} /var/opt/jfrog/worker/data/router/keys/trusted;cp -v ${file} /var/opt/jfrog/worker/etc/security/keys/trusted; fi done;
if [ -f /var/opt/jfrog/worker/data/router/keys/trusted/tls.crt ]; then mv -v /var/opt/jfrog/worker/data/router/keys/trusted/tls.crt /var/opt/jfrog/worker/data/router/keys/trusted/ca.crt; fi;
if [ -f /var/opt/jfrog/worker/etc/security/keys/trusted/tls.crt ]; then mv -v /var/opt/jfrog/worker/etc/security/keys/trusted/tls.crt /var/opt/jfrog/worker/etc/security/keys/trusted/ca.crt; fi;
{{- end -}}

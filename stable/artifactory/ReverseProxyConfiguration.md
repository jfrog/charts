...
*   Go to Artifactory Admin -> HTTP Settings. 
    ...
    Internal Hostname: $ARTIFACTORY_SERVICE_NAME
...
*   Deploy Artifactory using helm chart with auto configuration update disabled in nginx.
    You can achieve it by setting value to `true` for `nginx.env.skipAutoConfigUpdate` in [values.yaml](https://github.com/jfrog/charts/blob/master/stable/artifactory-ha/values.yaml#L362)
    Which sets Environment Variable `SKIP_AUTO_UPDATE_CONFIG=true` in Nginx container.
    ```bash
    helm install --name artifactory-ha --set nginx.env.skipAutoConfigUpdate=true jfrog/artifactory-ha
    ```

*   Create file called `artifactory.conf` with static nginx configurations. More [nginx configuration examples](https://github.com/jfrog/artifactory-docker-examples/tree/master/files/nginx/conf.d)  
    ```bash
    ###########################################################
...
*   Once Artifactory is deployed. Get Nginx pod name by running `kubectl get pods`.

*   Copy `artifactory.conf` file to Nginx pod using following command:
    ```bash
    kubectl cp ./artifactory.conf $NGINX_POD_NAME:/etc/nginx/conf.d/artifactory.conf
    ```

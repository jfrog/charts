...
```bash
helm upgrade --install artifactory --namespace artifactory center/jfrog/artifactory --verison 8.10.0
```
...
helm upgrade --install artifactory --namespace artifactory --set postgresql.enabled=false center/jfrog/artifactory
```
...
helm upgrade --install artifactory --namespace artifactory --set artifactory.replicator.enabled=true --set artifactory.replicator.publicUrl=<artifactory_url>:<replicator_port> center/jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --namespace artifactory center/jfrog/artifactory
```
...
```bash
helm upgrade <myrelease> center/jfrog/artifactory --set postgresql.postgresqlPassword=${POSTGRES_PASSWORD}
```
...
```bash
helm upgrade --install artifactory center/jfrog/artifactory -f values.yaml
``` 
...
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic center/jfrog/artifactory
...
```bash
helm upgrade --install artifactory center/jfrog/artifactory -f access-creds-values.yaml
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic,artifactory.configMapName=my-release-bootstrap-config center/jfrog/artifactory
```

OR

```bash
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.licenseKey=<LICENSE_KEY>,artifactory.configMapName=my-release-bootstrap-config center/jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --set nginx.customConfigMap=nginx-config center/jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --set imagePullSecrets=regsecret center/jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory -f plugins.yaml center/jfrog/artifactory
```
...
You have the option to add additional ingress rules to the Artifactory ingress. An example for this use case can be routing the /artifactory path to Xray.
...
```bash
helm upgrade --install artifactory --namespace artifactory center/jfrog/artifactory -f artifactory-values.yaml
```

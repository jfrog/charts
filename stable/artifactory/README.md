...
```bash
helm upgrade --install artifactory --namespace artifactory jfrog/artifactory --verison 8.11.5
```
...
helm upgrade --install artifactory --namespace artifactory --set postgresql.enabled=false jfrog/artifactory
```
...
helm upgrade --install artifactory --namespace artifactory --set artifactory.replicator.enabled=true --set artifactory.replicator.publicUrl=<artifactory_url>:<replicator_port> jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --namespace artifactory jfrog/artifactory
```
...
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic jfrog/artifactory
...
```bash
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic,artifactory.configMapName=my-release-bootstrap-config jfrog/artifactory
```

OR

```bash
helm upgrade --install artifactory --namespace artifactory --set artifactory.license.licenseKey=<LICENSE_KEY>,artifactory.configMapName=my-release-bootstrap-config jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --set nginx.customConfigMap=nginx-config jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory --set imagePullSecrets=regsecret jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory -f plugins.yaml jfrog/artifactory
```
...
```bash
helm upgrade --install artifactory --namespace artifactory jfrog/artifactory -f artifactory-values.yaml
```

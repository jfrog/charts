...
helm install --name artifactory --set postgresql.enabled=false center/jfrog/artifactory
```
...
helm install --name artifactory --set artifactory.replicator.enabled=true --set artifactory.replicator.publicUrl=<artifactory_url>:<replicator_port> center/jfrog/artifactory
```
...
helm install --name artifactory --namespace artifactory --set artifactory.persistence.customBinarystoreXmlSecret=custom-binarystore center/jfrog/artifactory
```
...
```bash
helm install --name artifactory --namespace artifactory --set postgresql.postgresqlPassword=12_hX34qwerQ2 center/jfrog/artifactory
```
...
helm install --name artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic center/jfrog/artifactory
...
```bash
helm install --name artifactory --set artifactory.license.secret=artifactory-license,artifactory.license.dataKey=art.lic,artifactory.configMapName=my-release-bootstrap-config center/jfrog/artifactory
```

OR

```bash
helm install --name artifactory --set artifactory.license.licenseKey=<LICENSE_KEY>,artifactory.configMapName=my-release-bootstrap-config center/jfrog/artifactory
```
...
```bash
helm install --name artifactory --set nginx.customConfigMap=nginx-config center/jfrog/artifactory
```
...
```bash
helm install --name artifactory --set imagePullSecrets=regsecret center/jfrog/artifactory
```
...
```bash
helm install --name artifactory -f plugins.yaml center/jfrog/artifactory
```

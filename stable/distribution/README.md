...
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade <myrelease> center/jfrog/distribution --set postgresql.postgresPassword=${POSTGRES_PASSWORD}
```
...
helm install --set distribution.masterKey=${MASTER_KEY} -n distribution center/jfrog/distribution
```

...
**NOTE:** Once Distribution is configured with [JFrog Mission-Control](../mission-control) users will need to copy token 
generated by Distribution service to Distributor using following commands:

```bash
# Copy token from Distribution Pod to local
kubectl cp ${DISTRIBUTION_POD_NAME}:/var/opt/jfrog/distributor/etc/security/token token

# Copy token from local to Distributor Pod
kubectl cp token ${DISTRIBUTOR_POD_NAME}:/var/opt/jfrog/distributor/etc/security/token
```

...
```bash
kubectl get secret <myrelease>-distribution-redis-secret -o json | jq '.data | map_values(@base64d)'
```
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade <myrelease> jfrog/distribution --set redis.password=<decoded_secret>
```

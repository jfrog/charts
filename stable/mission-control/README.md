...
Mission Control HA cluster uses a unique mc key. By default the chart has one set in values.yaml (`missionControl.masterKey`).
...
* Restart the new deployment
  ...
  ```
* A new master.key will be generated after this upgrade, save a copy of this key. **NOTE**: This should be passed on all future calls to `helm install` and `helm upgrade`!
```bash
export MC_KEY=$(kubectl exec -it <mission-control-pod> -n <new_namespace> -c mission-control -- cat / )
```
* Remove old release
...
| `missionControl.masterkey`                   | Mission Control masterkey . Can be generated with `openssl rand -hex 32` |``|

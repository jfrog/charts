...
If mission-control was installed without providing a value to postgresql.postgresPassword (a password was autogenerated), follow these instructions:
1. Get the current password by running:
```bash
kubectl get secret <myrelease>-postgresql -o json | jq '.data | map_values(@base64d)'
```
2. Upgrade the release by passing the previously auto-generated secret:
```bash
helm upgrade <myrelease> jfrog/mission-control --set postgresql.postgresPassword=<decoded_secret>
```
